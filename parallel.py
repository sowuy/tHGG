...
class SGEJob(LsfJob):
    ...
    def run(self,script):
        ...
        if mydomain == "hep.ph.ic.ac.uk":
            ...
            qsubCmdParts = [ "qsub",
                             "-q " + self.lsfQueue + " -l walltime=06:00:00 ",
                             ]
        ...
        if self.exitStatus != 0:
            ...
            for line in out.split("\n"):
                # kskovpen
                if "cream02.iihe.ac.be" in line:
                #if line.startswith("Your job"):
                    #self.jobid = int(line.split(" ")[2])
                    self.jobid = int(line.split(".")[0])
                    break
    ...
    def handleOutput(self):
        ...
        if self.async:
            result = commands.getstatusoutput("qstat -u kskovpen")
            checkcount = 0
...
class SGEMonitor(LsfMonitor):
    ...
    def monitor(self):
        status = commands.getstatusoutput("qstat -u kskovpen")
        checkcount = 0
        if (self.rebootMitigation):
